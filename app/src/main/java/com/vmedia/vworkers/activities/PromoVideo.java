...
import com.vmedia.vworkers.databinding.ActivityPromoVideoBinding;
...
public class PromoVideo extends AppCompatActivity{
    ActivityPromoVideoBinding bind;
    Activity activity;
    Pref pref;
    List<TaskResp> list;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bind =ActivityPromoVideoBinding.inflate(getLayoutInflater());
        Fun.statusbar(this);
        setContentView(bind.getRoot());

        activity= PromoVideo.this;
    ...
    private void getData() {
        Objects.requireNonNull(ApiClient.restAdapter(activity)).create(ApiInterface.class).apiPromo(js(activity,"PromoData", pref.User_id(), "","video","")).enqueue(new Callback<List<TaskResp>>() {
            @Override
